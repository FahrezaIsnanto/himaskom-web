<template>
   ...
   <div>
     ...
     <h1 class="mb-8 text-3xl font-bold">
       <Link class="text-indigo-400 hover:text-indigo-600" href="/admin/division">Bidang</Link>
       <span class="text-indigo-400 font-medium">/</span> Edit
     </h1>
     <div class="max-w-3xl bg-white rounded-md shadow overflow-hidden">
       <form @submit.prevent="update">
         <div class="flex flex-wrap -mb-8 -mr-6 p-8">
            <text-input v-model="form.title" :error="form.errors.title" class="pb-8 pr-6 w-full lg:w-1/2" label="Bidang" />
 ...
 export default {
   ...
   methods: {
    update() {
       this.form.post(`/admin/division/${this.division.id}/update`,{
        onError: (error)=>{
     ...
     destroy(){
        if (confirm('Apakah anda yakin akan menghapus?')) {
            this.$inertia.delete(`/admin/division/${this.division.id}`,{
                onError: (error)=>{
                    const message = (error.msg)?error.msg:'';
                    this.$notify({
                    title: "Terjadi Kesalahan!",
